...
Communication.prototype.start = function() {
    ...
    this.bindEvent()
    rpc.startServer({
        sendWorkUnit: self.receiveServerMessage.bind(self),
    })
}

Communication.prototype.startClient = function(serverInfo) {
    const self = this
    console.log('start client ---------> ', serverInfo)
...
Communication.prototype.bindEvent = function() {
    const self = this
    myDispatch.bind(config.events.serviceDiscover.newNodeOnline, {
        callback: self.startClient.bind(self),
        level: 1
    })
    myDispatch.bind(config.events.serviceDiscover.nodeOffline, self.removeConnector.bind(self))
}
